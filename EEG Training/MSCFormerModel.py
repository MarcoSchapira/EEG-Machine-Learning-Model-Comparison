...
class MSCFormer(nn.Module):
    def __init__(self, parameters, num_class, num_channel, **kwargs):
        super().__init__()
        self.cls_token = nn.Parameter(torch.randn(1, 1, parameters.emb_size))
        self.number_class, self.number_channel = num_class, num_channel
    ...
    def forward(self, x):
        ...
        #Addition: Expand the learnable token to match the current batch size 'b'
